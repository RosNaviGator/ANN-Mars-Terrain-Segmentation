...
def get_model(input_shape, num_classes, seed=42, dropout_rate=0.3):
    ...
    # Upsampling path using addition
    u1 = tfkl.UpSampling2D()(bottleneck)

    # Ensure down_block_2 and u1 have the same number of channels before addition
    u1 = tfkl.Conv2D(64, kernel_size=1, padding='same', name='align_channels_u1')(u1)  # Align the number of channels
    u1 = tfkl.Add(name='add_skip1')([u1, down_block_2])  # Use addition instead of concatenation
    u1 = unet_block(u1, 64, dropout_rate=dropout_rate, name='up_block1_')

    u2 = tfkl.UpSampling2D()(u1)

    # Ensure down_block_1 and u2 have the same number of channels before addition
    u2 = tfkl.Conv2D(32, kernel_size=1, padding='same', name='align_channels_u2')(u2)  # Align the number of channels
    u2 = tfkl.Add(name='add_skip2')([u2, down_block_1])  # Use addition instead of concatenation
    ...
    model = tf.keras.Model(inputs=input_layer, outputs=output_layer, name='Unet_skip_add')
    return model
