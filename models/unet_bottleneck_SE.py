...
def se_block(input_tensor, reduction_ratio=16, name=''):
    ...
    se = tfkl.Dense(channels // reduction_ratio, activation='relu', use_bias=False)(se)
    se = tfkl.Dense(channels, activation='sigmoid', use_bias=False)(se)
...
def get_model(input_shape, num_classes, seed=42, dropout_rate=0.3):
    ...
    model = tf.keras.Model(inputs=input_layer, outputs=output_layer, name='Unet_bottleneck_SE')
    return model
